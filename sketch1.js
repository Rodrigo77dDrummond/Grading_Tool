...
let s = "19. Commas and quotations. Explanation: It\'s a little counter intuitive, but commas that immediately follow quotations actually get pulled inside the quote marks. What to do: Put the comma first before the closing quote marks. Another note: Delete commas at the end of a quotation that are not required grammatically by the full sentence.                                                               "
...
console.log(config)
 ...
 database = firebase.database();
  ...
  var numbers = createInput("Feedback numbers");
  numbers.input(NUMBERS);
  numbers.size(175);
  numbers.position(displayWidth/2+5,180)

  var custom = createInput("Write specific comment here");
  custom.input(CUSTOM);
  custom.size(340);
...
//creates the Lick themed background as well as the descriptions of each input field, and details such as boxes
  ...
  noFill()
  strokeWeight(4)
  rect(displayWidth/2-190,45,390,240)
  fill(0)
  textSize(15)
...
//only when the comment is finally constructed with all the data needed, it prints it onto the page so it can be reviewed before being copied from database
  if(comment1){
    ...
    fill(0)
    textSize(15)
    text(comment2, displayWidth/2-625, 300, 1250, 50)
